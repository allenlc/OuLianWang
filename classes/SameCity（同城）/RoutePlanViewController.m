...
- (id)initWithModel:(HomeModel *)mm
{
    ...
    {
        self.model=mm;
        NSLog(@"%@ %@",mm.latitude,mm.longitude);
...
- (void)configMapView
{
    ...
    [self.mapView setRegion:MACoordinateRegionMake(CLLocationCoordinate2DMake(_startPoint.latitude, _startPoint.longitude), MACoordinateSpanMake(0.1, 0.1))];
...
- (void)initNaviPoints
{
    ...
    _startPoint = [AMapNaviPoint locationWithLatitude:[single.lan floatValue] longitude:[single.lon floatValue]];
    _endPoint   = [AMapNaviPoint locationWithLatitude:[self.model.latitude floatValue] longitude:[self.model.longitude floatValue]];
    
    NSLog(@"%f %f ------ %f %f",[single.lan floatValue],[single.lon floatValue],[_model.latitude floatValue],[_model.longitude floatValue]);
