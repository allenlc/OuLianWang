...
@interface JiaJuViewController ()<MAMapViewDelegate>
{
...
{
    [super viewDidLoad];
    self.searchBar.placeholder=@"搜索附近的家居用品";
    [self creatButton];
    _selectedArr=[NSMutableArray array];
    self.catId=0;//初始化参数为1
    self.big_catId=1;
    _page=1;
}
...
-(void)creatButton
{
    NSArray *photoArray=@[@"tcjiaju.png",@"tcjiazhuang.png",@"tcjiancai.png",@"tcjiasi.png"];
    NSArray *photoArray2=@[@"综合B_95.png",@"综合B_96.png",@"综合B_97.png",@"综合B_98.png"];
    NSArray *titleArray=@[@"家具",@"家装",@"建材",@"家饰"];
    for(int i=0;i<4;i++)
    {
        //创建button
...
-(void)startDownloadData
{
    ...
    //测试
    NSMutableString *urlString=[NSMutableString stringWithFormat:TC_JiaJu_URL,self.big_catId,self.catId,lon,lan,@"1",@"1",areaId,_page];
