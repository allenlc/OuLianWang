...
//===
-(void)setLiaotianModel:(LiaoTianModel *)model
{
    self.strIcon = model.headImg;
    self.strName = model.userID;
    self.strId = model.userID;
    self.strTime = model.sendTime;
    self.strContent=model.content;

    
    if ([model.from isEqualToString:@"0"]) {
        self.from=0;
    }
    else
        self.from=1;
