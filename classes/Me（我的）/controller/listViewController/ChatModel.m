...
#pragma mark —添加数据库的方法
-(void)addItemsFromDatabaseArray:(NSArray *)array
{
    for (int i=0; i<array.count; i++) {
        LiaoTianModel *mm=array[i];
    
        UUMessageFrame *messgeframe=[[UUMessageFrame alloc]init];
        UUMessage *message = [[UUMessage alloc] init];
        [message setLiaotianModel:mm];
        [messgeframe setMessage:message];
//        [message minuteOffSetStart:previousTime end:dataDic[@"strTime"]];
//        messageFrame.showTime = message.showDateLabel;
//        [messageFrame setMessage:message];
        
        [self.dataSource addObject:messgeframe];
    }
}
// 添加自己的item
...
- (void)addOtherItem:(NSDictionary *)dic
{
    ...
    [dataDic setObject:[[NSDate date] description] forKey:@"strTime"];
//    [dataDic setObject:@"Hi,Daniel" forKey:@"strName"];
    [dataDic setObject:URLStr forKey:@"strIcon"];
