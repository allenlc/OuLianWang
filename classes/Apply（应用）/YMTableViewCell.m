...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        _userNameLbl = [[UILabel alloc] initWithFrame:CGRectMake(10 + TableHeader + 5, 5, screenWidth - 120, TableHeader/2)];
        ...
        UIImageView  * botLine = [[UIImageView alloc]initWithFrame:CGRectMake(0, self.height-5, self.width, 1)];
        botLine.backgroundColor =ZQColor(214, 214, 214);
...
#pragma mark -  //头像 昵称 简介
    ...
    _userHeaderImage.layer.cornerRadius=0;
    _userHeaderImage.layer.masksToBounds=NO;
    _userHeaderImage.layer.borderColor=[UIColor whiteColor].CGColor;
    _userHeaderImage.frame=CGRectMake(10,3, 40, 40);
    
    _userNameLbl.text = tempDate.messageBody.posterName;
    _userNameLbl.textColor=ZQColor(25, 92, 238);
   // _userNameLbl.frame=CGRectMake(10, 10, 200, 40);
    
    _userIntroLbl.text = tempDate.messageBody.posterIntro;
    _userIntroLbl.textColor=ZQColor(25, 92, 238);
    _userIntroLbl.font=[UIFont  systemFontOfSize:12];
   // _userIntroLbl.frame=CGRectMake(10, 40, 200, 40);
...
#pragma mark - // /////////添加说说view
    ...
    WFTextView *textView = [[WFTextView alloc] initWithFrame:CGRectMake(10 + TableHeader + 5, 5 + TableHeader/2, screenWidth - 50 - TableHeader , 0)];
    ...
    textView.frame = CGRectMake(10 + TableHeader + 5, 5 + TableHeader/2, screenWidth - 35 - TableHeader , hhhh);
    
    [_ymShuoshuoArray addObject:textView];
    
    //按钮
    foldBtn.frame = CGRectMake(offSet_X - 5, 15 + 5 + hhhh + 10 , 50, 20 );
...
#pragma mark - /////// //图片部分
    ...
    for (int  i = 0; i < [ymData.showImageArray count]; i++) {
        ...
        UIImageView *image = [[UIImageView alloc] initWithFrame:CGRectMake(((screenWidth - 280)/4)*(i%3 + 1) +5 + TableHeader+73*(i%3), 5 + 5 * ((i/3) + 1) + (i/3) *  ShowImage_H + hhhh + kDistance + (ymData.islessLimit?0:30), ShowImage_H, ShowImage_H)];
...
#pragma mark - /////点赞部分
    ...
    favourView.textColor = ZQColor(25, 92, 238);
    [favourView setOldString:ymData.showFavour andNewString:ymData.completionFavour];
    favourView.frame = CGRectMake(55 + 30, 10 + ShowImage_H + (ShowImage_H + 10)*(scale_Y/3) + origin_Y + hhhh + kDistance + (ymData.islessLimit?0:30) + balanceHeight + kReplyBtnDistance, screenWidth - offSet_X * 2 - 30, ymData.favourHeight);
...
//点赞图片的位置
    _favourImage.frame = CGRectMake(55 + 8, favourView.frame.origin.y, (ymData.favourHeight == 0)?0:20,(ymData.favourHeight == 0)?0:20);
...
#pragma mark - ///// //最下方回复部分
    ...
    for (int i = 0; i < ymData.replyDataSource.count; i ++ ) {
        ...
        WFTextView *_ilcoreText = [[WFTextView alloc] initWithFrame:CGRectMake(55,  10 + ShowImage_H + (ShowImage_H + 10)*(scale_Y/3) + origin_Y + hhhh + kDistance + (ymData.islessLimit?0:30) + balanceHeight + kReplyBtnDistance + ymData.favourHeight + (ymData.favourHeight == 0?0:kReply_FavourDistance), screenWidth - offSet_X * 2, 0)];
        ...
        _ilcoreText.frame = CGRectMake(55, 10 + ShowImage_H + (ShowImage_H + 10)*(scale_Y/3) + origin_Y + hhhh + kDistance + (ymData.islessLimit?0:30) + balanceHeight + kReplyBtnDistance + ymData.favourHeight + (ymData.favourHeight == 0?0:kReply_FavourDistance), screenWidth - 65, [_ilcoreText getTextHeight]);
    ...
    if (ymData.replyDataSource.count == 0) {//没回复的时候
        ...
        replyImageView.frame = CGRectMake(55, backView_Y - 10 + balanceHeight + 5 + kReplyBtnDistance, 0, 0);
        _replyBtn.frame = CGRectMake(screenWidth - 45, 10 + ShowImage_H + (ShowImage_H + 10)*(scale_Y/3) + origin_Y + hhhh + kDistance + (ymData.islessLimit?0:30) + balanceHeight + kReplyBtnDistance - 24, 40, 18);
        
    }else{
        
        replyImageView.frame = CGRectMake(55, backView_Y - 10 + balanceHeight + 5 + kReplyBtnDistance-40, screenWidth - 65, backView_H + 20 - 8);//微调
        
        _replyBtn.frame = CGRectMake(screenWidth - 45, replyImageView.frame.origin.y - 24, 40, 18);
    }
    _botLine.frame = CGRectMake(0, CGRectGetMaxY(replyImageView.frame)+9, self.width, 1);
